...
TEST(ChessTests, GameInitTest) {
  ...
  auto state_provided = std::make_unique<Game>(
...
TEST(ChessTests, MoveFormatTest) {
  ...
  std::vector<std::string> invalid_inputs = {
      "AAAAAAAAAAAH", "Pe2xxxe4",
      "Pe27e2",       "",
      "\033",         "他们甚至无法找到的炸弹客 直到他自己的兄弟把他"};

  for (const auto &input : invalid_inputs) {
    ASSERT_FALSE(movemaker->valid(input))
        << "In MoveFormatTest: invalid format not recognized";
  }
  ...
  for (const auto &input : valid_inputs) {
    ASSERT_TRUE(movemaker->valid(input))
        << "In MoveFormatTest: valid format not recognized";
  }
}
...
TEST(ChessTests, MoveTests) {
  ...
  auto game = std::make_unique<Game>(
      "rnbqkbnrpppp ppp            p       P           P PP PPPRNBQKBNR");
  auto movemaker = std::make_unique<MoveFactory>();
...
TEST(ChessTests, CheckmateTest) {
  ...
  auto game = std::make_unique<Game>(
      "r  r  k   q bpp    p   p ppn     P BP   P     Q     RPPPR     K ");
  auto movemaker = std::make_unique<MoveFactory>();
  std::string move_inp = "Qg3xg7";
  auto move = movemaker->parse_move(move_inp);
  game->make_move(move);
  game->swap();
  ASSERT_TRUE(game->checkmate(game->to_move()))
      << "In CheckmateTest: checkmate not recognized";

  // checkmate after explosion (no king on board):
  auto game2 = std::make_unique<Game>(
      "r  r  k   q bpp    p   p ppn     P BP   P     Q     RPPPR       ");
  ASSERT_TRUE(game2->checkmate(Player::White))
      << "In CheckmateTest: checkmate by missing king not recognized";
}
...
TEST(ChessTests, CheckTest) {
  ...
  auto game = std::make_unique<Game>(
      "rn  kbnrpppPpppp                                PPP PPPPRNBQKBNR");
  game->swap();
  ASSERT_TRUE(game->in_check(game->to_move()))
      << "In CheckTest: check not recognized";
  ASSERT_FALSE(game->checkmate(game->to_move()))
      << "In CheckTest: check mistaken for checkmate (1)";

  // white in check by rook, king can evade
  auto game2 = std::make_unique<Game>(
      "     rk  p   ppppq   b                   P Q N  P    PPP   r K  ");
  ASSERT_TRUE(game2->in_check(game2->to_move()))
      << "In CheckTest: check not recognized";
  ASSERT_FALSE(game2->checkmate(game2->to_move()))
      << "In CheckTest: check mistaken for checkmate (2)";
}
...
TEST(ChessTests, PawnPromotionTest) {
  ...
  auto move = movemaker->parse_move(move_inp);
  ASSERT_FALSE(game->try_move(move))
      << "In PawnPromotionTest: invalid promotion allowed (cannot promote to "
         "pawn)";

  std::string promote_to_king = "Pd7d8=K";
  auto move_king = movemaker->parse_move(promote_to_king);
  ASSERT_FALSE(game->try_move(move_king))
      << "In PawnPromotionTest: invalid promotion allowed (cannot promote to "
         "king)";
  ...
  ASSERT_TRUE(game->try_move(valid_prom_move))
      << "In PawnPromotionTest: valid promotion not recognized";
}
...
TEST(ChessTests, GameplayTest) {
  ...
  std::vector<std::string> moves = {"Pe2e4", "pe7e5",  "Ng1f3",  "bf8c5",
                                    "Pd2d4", "bc5xd4", "Nf3xd4", "pe5xd4"};

  for (const auto &input : moves) {
    auto move = movemaker->parse_move(input);
    ASSERT_TRUE(game->try_move(move))
        << "In GameplayTest: valid move not recognized";
    game->make_move(move);
    game->swap();
  }
}
