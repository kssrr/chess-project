...
// Check recognition
// See if the game recognizes when a player is in check, and whether it can tell check from checkmate
...
TEST(ChessTests, PawnPromotionTest) {
  ...
  auto move = movemaker->parse_move(move_inp);
  ASSERT_FALSE(game->try_move(move)) << "In PawnPromotionTest: invalid promotion allowed (cannot promote to pawn)";

  std::string promote_to_king = "Pd7d8=K";
  auto move_king = movemaker->parse_move(promote_to_king);
  ASSERT_FALSE(game->try_move(move_king)) << "In PawnPromotionTest: invalid promotion allowed (cannot promote to king)";
