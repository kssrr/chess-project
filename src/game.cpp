...
Board Game::init_board() const {
  ...
  board[0] = {std::make_shared<Rook>(Player::Black),
              std::make_shared<Knight>(Player::Black),
              std::make_shared<Bishop>(Player::Black),
              std::make_shared<Queen>(Player::Black),
              std::make_shared<King>(Player::Black),
              std::make_shared<Bishop>(Player::Black),
              std::make_shared<Knight>(Player::Black),
              std::make_shared<Rook>(Player::Black)};
  ...
  board[7] = {std::make_shared<Rook>(Player::White),
              std::make_shared<Knight>(Player::White),
              std::make_shared<Bishop>(Player::White),
              std::make_shared<Queen>(Player::White),
              std::make_shared<King>(Player::White),
              std::make_shared<Bishop>(Player::White),
              std::make_shared<Knight>(Player::White),
              std::make_shared<Rook>(Player::White)};
...
Game::Game(const std::string &input) {
  ...
  state_ = board;
  current_player_ =
      Player::White; // white always starts, even when reading from file
}
    ...
    for (size_t j = 0; j < 8; ++j) {
      ...
      if (ptr)
        std::cout << " " << (ptr->owner() == Player::Black ? BLACK : WHITE)
                  << (char_view ? std::string(1, ptr->to_char())
                                : ptr->unicode())
                  << " " << RESET_BG;
      else
        std::cout << "   " << RESET_BG;
...
void Game::show(bool char_view) const {
  ...
  std::cout << (this->to_move() == Player::White ? "White" : "Black")
            << "'s turn.\n";
...
void Game::swap() {
  this->current_player_ == Player::White
      ? this->current_player_ = Player::Black
      : this->current_player_ = Player::White;
}
...
bool Game::substantively_valid(std::shared_ptr<Move> move,
                               bool threat_check = false) const {
  ...
  if (ref_piece != piece_at_start->to_char())
    return false; // referenced piece not at starting loc.

  if (move->has_capture() && !piece_at_dest)
    return false; // marked as capture but no piece at dest

  if (!threat_check &&
      (move->has_capture() && piece_at_dest->owner() == current_player_))
    return false; // piece to capture belongs to moving player

  if (!piece_at_start->valid(move, state_))
    return false; // piece cannot move like this

  // Check pawn promotion: (this should ideally be done in Pawn::valid)
  if (move->is_promotion()) {
    if (ref_piece != 'P' && ref_piece != 'p')
      return false; // only pawns can be promoted

    int promotion_row = (piece_at_start->owner() == Player::White) ? 0 : 7;
    if (to.row != promotion_row)
      return false; // promotion move has to end up in opposing back row

    // promotion cannot change the owner of the piece:
    Player owner_after_promotion =
        std::isupper(move->promote_to()) ? Player::White : Player::Black;
    if (piece_at_start->owner() != owner_after_promotion)
      return false;

    if (move->promote_to() == move->piece_char())
      return false; // piece has to be promoted (cannot remain itself)

    if (move->promote_to() == 'k' || move->promote_to() == 'K')
      return false; // cannot promote to become a king
  }
    ...
    for (int col = 0; col < 8; ++col) {
      if (state_[row][col]) {
        if (state_[row][col]->owner() != p) {
          ...
          auto king_attack = std::make_shared<Move>(
              current_piece->to_char(), Field(row, col), king_field, true);

          // if piece can perform valid capture move on king, king is in check
          // (mark as threat_check):
          if (substantively_valid(king_attack, true))
            return true;
        }
      }
    }
...
bool Game::try_move(std::shared_ptr<Move> move) {
  if (!substantively_valid(move, false))
    return false;
  ...
  for (int row = 0; row < 8; ++row) {
    for (int col = 0; col < 8; ++col) {
      ...
      for (int r = 0; r < 8; ++r) {
        for (int c = 0; c < 8; ++c) {
          if (r == row && c == col)
            continue; // Skip the same square

          auto move =
              std::make_shared<Move>(piece->to_char(), Field(row, col),
                                     Field(r, c), state_[r][c] != nullptr);
...
void Game::print_moves(const std::string &input, const bool char_view) {
  ...
  std::cout << (this->to_move() == Player::White ? "White" : "Black")
            << "'s turn.\n";
  std::cout
      << "Commands: (:n)ew game (:u)ndo (:q)uit (:t)oggle character mode\n";
  std::cout << "\033[43m" << "Input>" << RESET_BG;
}
...
void Game::get_bomber(Player p, bool char_view) const {
  ...
  std::regex valid_regex(p == Player::White
                             ? "^[BNPQR][a-h][1-2]$"
                             : "^[bnpqr][a-h][7-8]$"); // no king allowed
  std::string input;

  std::cout << (p == Player::White ? "White" : "Black")
            << "'s suicide bomber:>";
  ...
  while (std::getline(std::cin, input)) {
    ...
    if (ptr->to_char() != piece_char) {
      std::cout << "That is not the piece at the location, that piece is "
                << ptr->to_char() << ". Try again\n>";
      continue;
    }
...
void Game::boom(Player p) {
  ...
  if (!found) {
    std::cout << "No bomb carrier for player "
              << (p == Player::White ? "white" : "black") << '\n';
    return;
  }
